...
const STEPS = [
  {
    ...
    description: "Enter your name, email, and contact details.",
    Icon: User,
  },
  {
    id: 2,
  ...
  },
  {
    ...
    description: "Provide details about your health center.",
    Icon: Building2,
  },
  {
    id: 4,
    title: "Location",
    description: "Pin your barangay health center on the map.",
    Icon: MapPin,
  },
  {
    id: 5,
    title: "Verification Documents",
    description: "Upload your ID and accreditation documents.",
    Icon: FileText,
  },
  {
    id: 6,
...
export function SignupStepper() {
  ...
  return (
    <form ref={formRef} action={registerBhw} className="space-y-0">
      {/* ── Progress Stepper ── */}
      <div className="mb-8">
            ...
            return (
              <div key={s.id} className="flex flex-1 items-center">
                <div className="flex flex-col items-center">
                  <div
                    className={cn(
                      "flex size-9 items-center justify-center rounded-full border-2 text-sm font-bold transition-all duration-200",
                      isCompleted
                  ...
                  <span
                    className={cn(
                      "mt-1.5 hidden text-[10px] font-medium leading-tight text-center sm:block max-w-[60px]",
                      isActive
      ...
      {/* ── Step header ── */}
      <div className="mb-6 rounded-2xl bg-gradient-to-br from-[#EFF6FF] to-[#F0FDFA] border border-[#BFDBFE]/60 px-6 py-5 dark:from-[#172338]/50 dark:to-[#0F2B28]/50 dark:border-slate-700/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-[#2563EB] dark:text-[#60A5FA]">
      ...
      {/* ── Error banner ── */}
      {error && (
        <div className="mb-4 flex items-start gap-3 rounded-2xl border border-[#FECACA] bg-[#FEF2F2] px-4 py-3 text-sm text-[#DC2626] dark:border-[#DC2626]/30 dark:bg-[#DC2626]/10 dark:text-[#FCA5A5]">
      ...
      <div className={step === 1 ? "space-y-5" : "hidden"}>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-[#64748B] dark:text-slate-400">
            You will use this email to log in after your account is approved.
          </p>
        </div>
        <div className="space-y-2">
      ...
      <div className={step === 2 ? "space-y-5" : "hidden"}>
        ...
        <div className="space-y-2">
          <label className={LABEL_CLS} htmlFor="bhwAccreditationNumber">
            BHW Accreditation Number{OPT}
          </label>
          <input
            className={INPUT_CLS}
            id="bhwAccreditationNumber"
            name="bhwAccreditationNumber"
            placeholder="e.g. BHW-2024-001234"
            type="text"
            value={bhwAccreditationNumber}
            onChange={(e) => setBhwAccreditationNumber(e.target.value)}
          />
        </div>
        <div className="space-y-2">
          <label className={LABEL_CLS} htmlFor="yearsOfService">
            Years of Service{OPT}
          </label>
          <input
            className={INPUT_CLS}
            id="yearsOfService"
      ...
      <div className={step === 3 ? "space-y-5" : "hidden"}>
        <div className="space-y-2">
          <label className={LABEL_CLS} htmlFor="healthCenterName">
            Health Center Name{OPT}
          </label>
          <input
            className={INPUT_CLS}
            id="healthCenterName"
            name="healthCenterName"
            placeholder="e.g. Barangay Health Center San Jose"
            type="text"
        ...
        <div className="space-y-2">
          <label className={LABEL_CLS} htmlFor="fullAddress">
            Full Address{OPT}
          </label>
          <textarea
            ...
            id="fullAddress"
            name="fullAddress"
            placeholder="e.g. 123 Rizal Street, Barangay San Jose, Quezon City, Metro Manila"
            rows={3}
            value={fullAddress}
            onChange={(e) => setFullAddress(e.target.value)}
          />
          <p className="text-xs text-[#64748B] dark:text-slate-400">
            Province, municipality, and barangay will be set using the location picker on the next step.
          </p>
        </div>
      </div>

      {/* ═══════════════════════════════════════════════════════════════════
          STEP 4 — Location (MapLocationPicker)
      ═══════════════════════════════════════════════════════════════════ */}
      ...
      <div className={step === 5 ? "space-y-5" : "hidden"}>
        <div className="rounded-2xl border border-[#FEF3C7] bg-[#FFFBEB] px-4 py-3 dark:border-[#F59E0B]/30 dark:bg-[#F59E0B]/10">
          <div className="flex items-start gap-2 text-sm text-[#92400E] dark:text-[#FCD34D]">
            ...
            <p>
              Upload clear, readable documents. Accepted formats: PDF, JPG, PNG. Maximum 5 MB per file.
            </p>
          </div>
        </div>

        <div className="space-y-2">
          <label className={LABEL_CLS} htmlFor="proofDocument">
            Valid ID or BHW Certificate
          </label>
          <input
            accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/*"
            className="w-full rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] px-4 py-3 text-sm text-[#1E293B] file:mr-4 file:rounded-xl file:border-0 file:bg-[#2563EB] file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-[#1D4ED8] dark:border-white/10 dark:bg-white/5 dark:text-slate-100 dark:file:bg-[#2563EB] cursor-pointer"
            id="proofDocument"
            name="proofDocument"
            required
            type="file"
            onChange={(e) =>
              setProofDocumentName(e.target.files?.[0]?.name ?? null)
            }
          />
          <p className="text-xs text-[#64748B] dark:text-slate-400">
            Upload a valid government-issued ID, BHW accreditation, or health center endorsement letter.
          </p>
        </div>

        <div className="space-y-2">
          <label className={LABEL_CLS} htmlFor="secondDocument">
            Supporting Document{OPT}
          </label>
          <input
            accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/*"
            className="w-full rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] px-4 py-3 text-sm text-[#1E293B] file:mr-4 file:rounded-xl file:border-0 file:bg-[#64748B] file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-[#475569] dark:border-white/10 dark:bg-white/5 dark:text-slate-100 dark:file:bg-[#475569] cursor-pointer"
            id="secondDocument"
            name="secondDocument"
            type="file"
            onChange={(e) =>
              setSecondDocumentName(e.target.files?.[0]?.name ?? null)
            }
          />
          <p className="text-xs text-[#64748B] dark:text-slate-400">
            You may upload one additional document to support your application.
          </p>
        </div>
      </div>
      ...
      {/* ── Navigation buttons ── */}
      <div className="mt-8 flex items-center justify-between gap-4 border-t border-[#E2E8F0] pt-6 dark:border-slate-700">
        {step > 1 ? (
          <Button
            type="button"
            variant="outline"
